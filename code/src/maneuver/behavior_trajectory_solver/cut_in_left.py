...
def cut_in_left(sim, ego, npc, forward, right, num_points, n, m, k):
    """
    生成向左变道路径点。
...
def calculate_velocity(ego, npc, n_forward, distance):
    ...
    t0 = (n_forward - (npc_length / 2)) / max(ego.state.speed, 0.1)
    t1 = (n_forward + (npc_length / 2)) / max(ego.state.speed, 0.1)
