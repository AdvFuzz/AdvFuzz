...
def get_distance_ego_npc2(ego, npc):
    ego_bbox = get_bbox2(ego,ego.bounding_box)
    npc_bbox = get_bbox2(npc,npc.bounding_box)
...
def compute_danger_fitness(ego, npc, yellow_line_points, collision=False):
    if collision:
        ...
        distance_yellow = (get_distance_ego_line(ego, yellow_line_points) + 1) ** 2
        distance_fitness = speed_norm / distance_yellow
        fitness = fitness + distance_fitness
