...
class Runner(object):
    ...
    def __init__(self, scenario_env_json, output_path, total_sim_time, default_record_folder, data_prime, lgsvl_map = 'SanFrancisco_correct', apollo_map = 'SanFrancisco_correct'):
        self.global_id = 0
        ...
        self.Status_FOLDER = 'status'
        self.COLLISION_FOLDER = 'collision'
        ...
        self.status_path = os.path.join(output_path, 'simulation', self.Status_FOLDER)
        self.collision_path = os.path.join(output_path, 'simulation', self.COLLISION_FOLDER)
        
        clear_and_create(self.scenario_path)
        clear_and_create(self.result_path)
        clear_and_create(self.record_path)
        clear_and_create(self.status_path)
        clear_and_create(self.collision_path)

        self.sim = Simulator(self.default_record_folder, self.record_path, total_sim_time, data_prime, self.status_path, self.collision_path, lgsvl_map, apollo_map) # save record to records/scenario_name/scenario_id
